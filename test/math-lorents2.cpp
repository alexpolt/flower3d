...
int main() {
  ...
  auto zs = sin( math::radians(3.0*z) ), zc = cos( math::radians(3.0*z) );
  auto sa = sin( math::radians(3.0*a) ), ca = cos( math::radians(3.0*a) );

  auto v = $normalize( vec4d{ zc*ca, zc*sa, zs, 0 } );

  auto m = mat4d {

    vec4d{  v[0], -v[1],  v[3], -v[2] },
    vec4d{  v[1],  v[0], -v[2], -v[3] },
